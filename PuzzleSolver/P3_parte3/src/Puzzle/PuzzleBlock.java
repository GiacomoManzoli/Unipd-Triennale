...
//Visibilità package
class PuzzleBlock implements PuzzleElement, Serializable {
    ...
    * passato per parametro */
    public PuzzleElement attach(PuzzleElement item) {
        ...
        if (canAttachVertically(item)) {
            /*Sono sicuro che item sia un blocco di righe o una riga completa perché c'è canAttachVertically testa che
            * il numero di colonne di questo blocco sia uguale al numero di colonne dell'item ricevuto come parametro*/

            /*Edit: item è per forza un blocco di righe perché adesso appena una riga diventa completa viene wrappata
            * in un blocco di righe*/

            /*List<PuzzleElement> items=item.getComponents();/**/

            List<PuzzleElement> items;
            if (item.isComplete()){

                items = new ArrayList<PuzzleElement>();
                items.add(item);
            } else{
                //è un blocco di righe
